...
class tenthTrainDataloader(torch.utils.data.Dataset):
    def __init__(self, opt, for_metrics):
        ...
        self.images, self.ren, self.labels, self.paths = self.list_images()
    ...
    def __getitem__(self, idx):
        ...
        rendered = Image.open(os.path.join(self.paths[1], self.ren[idx])).convert('RGB')
        label = Image.open(os.path.join(self.paths[2], self.labels[idx]))
        image, rendered, label = self.transforms(image, rendered, label)
        #label = label * 255
        return {"image": image, "rendered": rendered, "label": label, "name": self.images[idx], "rendered_name": self.ren[idx]}

    def list_images(self):
        mode = "test" if self.opt.phase == "test" or self.for_metrics else "train"
        path_img = os.path.join(self.opt.dataroot, mode, "images")
        path_ren = os.path.join("realistic_render", "trainA")
        path_lab = os.path.join(self.opt.dataroot, mode, "masks")
        img_list = os.listdir(path_img)
        ren_list = os.listdir(path_ren)
        lab_list = os.listdir(path_lab)
        img_list = [filename for filename in img_list if ".png" in filename or ".jpg" in filename]
        ren_list = [filename for filename in ren_list if ".png" in filename or ".jpg" in filename]
        ren_list = ren_list[:len(img_list)]
        ...
        return images, ren, labels, (path_img, path_ren, path_lab)

    def transforms(self, image, rendered, label):
